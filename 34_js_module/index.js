...
const product = [
  ...
  product.map((ele) => {
      container.innerHTML += `<div class="col-3"> ${card(ele.image, ele.title, ele.price)} </div>`
  })
  
  user.map((ele) => {
      container.innerHTML += `<div class="col-3"> ${card(ele.picture.large, ele.name.title)} </div>`
  })
